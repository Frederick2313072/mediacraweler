        ...
        with open(os.path.join(output_dir, 'index.md'), 'w', encoding='utf-8') as f:
            ...
            for i, content in enumerate(contents_data, 1):
                try:
                    ...
                    if not note_id:
                        print(f"跳过没有note_id的内容: {content}")
                        continue
                    ...
                    with open(content_file, 'w', encoding='utf-8') as cf:
                        ...
                        # 写入内容信息
                        cf.write('## 基本信息\n\n')
                        cf.write(f'- 作者：{content.get("nickname", "匿名用户")}\n')
                        cf.write(f'- 发布时间：{datetime.fromtimestamp(content.get("time", 0)/1000).strftime("%Y-%m-%d %H:%M:%S")}\n')
                        cf.write(f'- 点赞数：{content.get("liked_count", 0)}\n')
                        cf.write(f'- 收藏数：{content.get("collected_count", 0)}\n')
                        ...
                        # 写入内容正文
                        cf.write('## 正文\n\n')
                        cf.write(f'{content.get("desc", "无内容")}\n\n')
                    ...
                    # 在索引文件中添加链接
                    f.write(f'- [{content.get("title", "无标题")}](content_{note_id}.md)\n')
